...
//        mBaiduMap.hideInfoWindow();
    ...
    private void resultDeal(SearchResult result) {
        ...
        if (result == null) {
            //没有找到地址
            ToastUtils.show(NewRoutePlanActivity.this, "没有找到地址,请点击地图选择终点位置");
            return;
        }
        ...
        if (result.error == SearchResult.ERRORNO.ST_EN_TOO_NEAR) {
            ToastUtils.show(NewRoutePlanActivity.this, "目的地太近没有搜索到线路，请尝试其他搜索方式");
            return;
        }
        if (result.error == SearchResult.ERRORNO.RESULT_NOT_FOUND) {
            ToastUtils.show(NewRoutePlanActivity.this, "没有找到可规划的路径");
            return;
        }
